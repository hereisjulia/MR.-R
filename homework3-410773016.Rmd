...
假設你叫王小明，學號41078392。
  ...
  2.本文檔開始的Frontmatter中，studentID（即鍵）其值目前為“你的學號”，以上面學號為例則必需改成“41078392”;而studentName（鍵）其值目前為“你的名字“，以上面名字為例則必需改成”王小明“
  
>每次作業滿分10分：收到谷歌行事歷邀請按是（接受）得2分，有寫作業於期限內上傳得3分，依作業準確率決定最後5分得分多寡。
>前述存檔UTF-8格式要求與frontmatter要求缺任何一個則扣1分。
> !!! 不使用作業檔作答（例如：作業1有同學拿自己上課練習檔在後面接上作業檔內容;也有同學用新文件產生rmd檔懶得刪除新檔案原有文字而在後面亂貼作業檔內容），其答案全部都會算錯（影響改題程式判讀）。
...
## 作答需知
每一題會有設好的ans chunk，它會有ansxx標籤（如下的ans99），請將你的程式寫在裡面。每一題都需要你把答案存在指定的物件，並在「**最後一行** “打物件名稱。以下例，如答案是`2 + 3`並要你存在答案物件test裡，那你的程式如下：
```{R}
test  < -  2 + 3  ＃答案存在指定測試物件
test  ＃最後一行打指定物件名稱
```
...
```{r ans25}
total <- sum(df_allShows$sourceWebName == "全國藝文活動資訊系統")
totalCount <- as.numeric(total)
class(totalCount)
totalCount
```
